...
export default function NewClientPage() {
  ...
  const fetchLawyers = async () => {
    try {
      ...
      const response = await getUsers({ role: 'LAWYER', limit: 1000, isActive: 'true' });
      setLawyers(response.users.map(u => ({
        id: u.id,
        name: u.name,
        email: u.email,
      })));
      ...
      toast({
        title: 'Error',
        description: error.message || 'Failed to load lawyers',
        variant: 'destructive',
      });
    } finally {
    ...
    return (
      <MainLayout title="Access Denied">
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">You don't have permission to create clients.</p>
        </div>
      </MainLayout>
    );
  }
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange('assignedLawyerId', value === 'NONE' ? '' : value)}
